...
import { FiTrash2, FiRefreshCw, FiSearch, FiDownload } from 'react-icons/fi'
...
export default function HeadHrCandidates() {
  ...
  const filtered = candidates.filter((c) => {
    ...
    return (
      ...
      c.jobs_applied?.toLowerCase().includes(q)
    )
  })
  ...
  return (
    <PanelShell>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="org-page-subtitle">
            {candidates.length} applicant{candidates.length !== 1 ? 's' : ''} who applied to one or more jobs
          </p>
        </div>
      ...
      {error && <div className="org-error-banner">{error}</div>}
      ...
      {loading ? (
        ...
        <div className="org-empty-state">
          {search ? 'No applicants match your search.' : 'No applicants yet. When someone applies to a job, they appear here.'}
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="org-table-head">
                  ...
                  <th className="org-th">Applications</th>
                  <th className="org-th">Jobs applied</th>
                  <th className="org-th">Last applied</th>
                  {!readOnly && <th className="org-th text-right">Actions</th>}
                </tr>
              </thead>
                  ...
                  <tr
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault()
                        navigate(`${basePath}/candidates/${encodeURIComponent(c.cid)}`)
                      }
                    }}
                    className="org-table-row-clickable"
                  >
                    <td className="px-4 py-3 font-mono text-xs text-[#55B9FF]">{c.cid}</td>
                    <td className="org-td-primary">{c.full_name || c.name || '—'}</td>
                    <td className="org-td-secondary">{c.email || '—'}</td>
                    <td className="org-td-secondary tabular-nums">{c.application_count ?? 0}</td>
                    <td className="org-td-muted text-xs max-w-[220px] truncate" title={c.jobs_applied || ''}>
                      {c.jobs_applied || '—'}
                    </td>
                    <td className="org-td-muted">{formatDate(c.last_applied_at)}</td>
                    {!readOnly && (
                      <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); setConfirmDelete(c) }}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-red-400 hover:bg-red-500/10 border border-red-500/30 transition-all"
                        >
