...
export default function SignupApplicant() {
    ...
    return (
        <AuthPageLayout
            ...
            subtitle={step === 1 ? 'Apply for jobs and track your applications.' : `We sent a code to ${email}. Enter it below.`}
        >
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50 shadow-premium p-6 sm:p-8">
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    {step === 1 ? 'Create your applicant account to apply for jobs' : `We sent a verification code to ${email}. Please enter it below.`}
                </p>
                {step === 1 ? (
                    <form onSubmit={onSubmit} className="mt-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input className="input-premium" placeholder="Your name" value={name} onChange={(e) => setName(e.target.value)} required />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Email</label>
                            <input type="email" className="input-premium" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
                        </div>
                        <div>
                        ...
                        <button type="submit" disabled={submitting} className="w-full rounded-xl bg-primary dark:bg-accent-blue text-white font-semibold py-3 shadow-md hover:opacity-90 transition-opacity disabled:opacity-70">
                    ...
                    <form onSubmit={onVerifyOTP} className="mt-6 space-y-4">
                        ...
                        <button type="submit" disabled={submitting || otp.length !== 6} className="w-full rounded-xl bg-primary dark:bg-accent-blue text-white font-semibold py-3 shadow-md hover:opacity-90 transition-opacity disabled:opacity-70">
                            {submitting ? 'Verifying...' : 'Verify & Create Account'}
                        </button>
                        <div className="flex gap-2">
                            <button type="button" onClick={async () => { setError(''); setResending(true); try { const res = await resendApplicantOTP({ email, phone }); if (res.ok) { setError(''); alert('OTP resent successfully! Please check your email.'); } else { setError(res.message || 'Failed to resend OTP'); } } catch (err) { setError(err?.message || 'Failed to resend OTP'); } finally { setResending(false); } }} disabled={resending} className="flex-1 text-sm text-accent-blue hover:underline disabled:opacity-50">
                        ...
                        {created && <div className="mt-3 text-sm text-emerald-600 dark:text-emerald-400">Account created successfully! Redirecting to login...</div>}
                    </form>
                )}
            </div>
        </AuthPageLayout>
