...
**Bulk resume parse (Current):** One dashboard row per bulk job — **Bulk Parse · N resumes**. Step timings are averages across files in that job; skipped steps show only as Skipped (no ms). Cache / Store Raw / Layout are typically skipped on the bulk text path. A per-file list appears under the checklist.
