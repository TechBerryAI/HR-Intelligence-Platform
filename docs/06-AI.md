...
- **Experience quality (Current):** sanitize drops edu-table headers, geo-only rows, duty-bleed roles; em-dash `Role — Company` supported; years recomputed after sanitize; Excel flatten avoids `at ()` / city-at-country noise
- **Location quality (Current):** reject pipe/phone/section-header locations; heal `Company | City` → city; strip `+91…⋄City` bleed
