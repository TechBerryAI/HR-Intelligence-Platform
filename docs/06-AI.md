...
- **Bulk Excel (Current):** deterministic skip-LLM requires `score_resume_toon(..., source_text=)` pass — fails when Experience/Internship section exists but experience empty; year-soup phones stripped on flatten; `ParseStatus=partial` when section evidence remains without Experience after LLM
- Gold / production gates: `tests/backend/document_intelligence/` + `ai/eval/resume_production_validation/` (Phase 3: `test_bulk_quality_phase3.py`)
