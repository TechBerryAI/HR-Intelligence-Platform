...
- **Education (Current):** multi-line institution↔degree coalesce; stronger section end bounds; duty-line pollution filters; academic/internship header aliases
...
- **Experience quality (Current):** sanitize drops edu-table headers, geo-only / institution-as-job rows, duty-bleed roles (aligned verb list); **Phase 6:** noun-led KPI/duty fragments (`Trends, and Revenue KPIs…`) rejected by `is_plausible_job_title` / `_is_bullet_or_duty_line` / `validate_role`; `Role —/–/- Company` and Company\|City → `ExperienceEntry.location`; years recomputed after sanitize from dated ranges and grounded prose (`Total Experience: N years`); Excel `_flatten_toon` safety-net recomputes years from exp `from`/`to` **or description date ranges** when `total_experience_years` is empty
- **Location quality (Current):** reject pipe/phone/section-header / skill-summary pollution; heal `Company | City` and `+91…⋄City` → city; **Phase 6:** shared city allowlist (`known_location_cities`) + aliases (`Nasik`→`Nashik`); recovery peels `experience[].location` → education institution cities → institute map (VIT→`Vellore`); section-aware early-body recovery (not from Skills/Summary)
...
- **Bulk formats (Current):** staging accepts **pdf / docx / png / jpg / jpeg / webp / tif / tiff**; legacy `.doc` is rejected with `unsupported_format` (convert to PDF/DOCX). Scanned PDF + image resumes use the same RapidOCR path and `BULK_OCR_RETRY_DPI` garbage/thin-text retry as single parse. Install OCR via `pip install -r requirements.txt` (`rapidocr-onnxruntime`; Python 3.10–3.12 recommended). Bulk Excel sets `ParseStatus=partial` with named `coverage_gaps=…` ParseNotes when core coverage still has evidence gaps. **Phase 6 bulk gate:** refuse deterministic skip when experience titles fail `is_plausible_job_title` or the Experience slice looks OCR-mushy; engine LLM path is wrapped with `_llm_semaphore` (`OLLAMA_MAX_CONCURRENT`). Offline rescore of the `f8f0bf2f` 52-PDF staging set (LLM off): location **90%** (was ~83%), Saloni duty-as-title gone / `Nashik` recovered, Adawet→`Vellore`, Sagar→`Bhubaneswar`; fresher Projects-only Experience stays empty by design — **re-run full bulk** for a fresh Excel with LLM.
...
**Bulk resume parse (Current):** One dashboard row per bulk job — **Bulk Parse · N resumes** (Bulk filter only). The list row has a chevron to preview filenames; the detail panel lists each resume with an expandable dropdown showing the **same pipeline steps as a single resume parse**. Cache lookup is skipped for throughput. **Layout runs when `RESUME_LAYOUT_ENABLED`** (default true). Extract strips NUL bytes; OCR DPI retry covers PDF + image extensions (`png/jpg/jpeg/webp/tif/tiff`) aligned with single parse. Accepted uploads: PDF, DOCX, and those images — **not** legacy `.doc`. Excel prefers Form DTO for **Current/Preferred Location** and education; `ParseStatus=partial` + named coverage gaps in ParseNotes when evidence remains unfilled. **Phase 6:** refuse deterministic skip on implausible experience titles or OCR-mushy Experience slices; LLM calls use `_llm_semaphore` (`OLLAMA_MAX_CONCURRENT`). Bulk does **not** write into the single-parse `parsed_resumes` cache — re-uploading the same PDF on Apply / single parse always runs a fresh parse (existing bulk-seeded rows are ignored by cache lookup). **After Phase 6 detail-field fixes, re-run bulk** to refresh Excel (pre-fix exports stay stale).
