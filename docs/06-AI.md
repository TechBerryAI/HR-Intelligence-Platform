...
- **Skills section capture (Current):** Required/Preferred Skills are read as full section blocks (not a single line); skill-section phrases up to ~8 words are kept; longer skill sentences contribute embedded tech tokens instead of being dropped
- **Keywords grounding (Current):** Keywords come from explicit profile keywords ∪ mandatory/preferred skills; whole-document tech/acronym scrape runs only when skills are empty
...
- Golden regression: `tests/backend/test_jd_golden_accuracy.py` (+ `fixtures/jd_gold/` including table KV, multi-column, unlabeled paragraph, title cases, detailed skills / soft-wrap bullets)
