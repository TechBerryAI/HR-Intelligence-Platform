...
- Pipeline: Document Intelligence — extract → layout/sections → deterministic parse → knowledge → residual semantic LLM → validate → store (TOON + `ApplicationFormDTO`)
- **Autofill (Current):** backend Form DTO only (`map_candidate_to_form` → FE `takeResumeFormDTO`). Legacy FE `mapResumeTOONToForm` is not used on the apply path.
- **preferredLocation (Current):** when preferred is empty and current location is present, Form DTO sets preferred = current (matches apply-form UX). Explicit “Preferred location: …” / relocate lines are extracted when present on the resume.
- **Education (Current):** multi-line blocks, single-line `Degree, Institution, Year`, orphan degree/institution coalescing; mapper requires both degree and institution (no invented `"Education"` placeholder).
- **Experience (Current):** duty/bullet lines never become jobs; company-first layouts (`Company` / `Role` / `dates`) normalize to Role–Company–dates headers before wrap-join; empty Experience section stays empty (no Projects scrape).
- Includes education present on resume (e.g. 10th/12th) when both sides parse cleanly.
- **LLM / semantic (Current):** `RESUME_SKIP_LLM_WHEN_DETERMINISTIC` defaults **false** — semantic enrichment always runs (not skipped when identity + body look complete). Bulk uses `BULK_SKIP_LLM_WHEN_DETERMINISTIC` (default false).
- **Parse reuse (Current):** `DOCUMENT_INTELLIGENCE_USE_CACHE` defaults **false** — re-uploading the same file always re-parses; TOON is still stored for ATS/apply.
...
- **Coverage gate (Current):** after deterministic parse (and again after repair), evidence in source vs filled fields is checked; missing-but-present fields are recovered from the JD text only (no invention). Location accepts unlabeled forms (`Location Mumbai`, title-line `– Mumbai`) and known-city fallbacks; unlabeled captures disallow newline bleed; implausible location noise (`Mumbai Work from offi`, duty fragments like `and job scheduling`) is rejected via `is_plausible_jd_location` so coverage cannot mark garbage as `filled`. Skills reject garbage (`JOB`), strip PDF `o ` letter-bullets, prefer Primary Technology labels, and replace non-skill-like tokens. Salary rejects currency-only noise (`rs`). Statuses (`filled` / `recovered` / `missing_with_evidence` / `missing_no_evidence`) are returned on form `coverage` and API `missing_fields`
- **Structural repair** (`repair_jd_toon`) always runs on API and in-memory parse paths
- **LLM / semantic (Current):** by default `JD_SKIP_LLM_WHEN_DETERMINISTIC=false` so residual semantic enrichment always runs after deterministic parse (even when title/skills look strong). Set the flag `true` to restore skip-when-deterministic. `force` never bypasses `DOCUMENT_INTELLIGENCE_SEMANTIC_AI=false`. Timeout via `DOCUMENT_INTELLIGENCE_SEMANTIC_TIMEOUT_SEC` (default 25s)
...
- **Parse reuse (Current):** `DOCUMENT_INTELLIGENCE_USE_CACHE` defaults **false** — each resume/JD upload runs a fresh pipeline (no `parsed_*` cache hit). Results are still **persisted** for ATS/apply. Set `true` to re-enable content-hash reuse. Optional `DOCUMENT_INTELLIGENCE_CACHE_TAG` invalidates stale rows when cache is on.
- Golden regression: `tests/backend/test_jd_golden_accuracy.py` (+ `fixtures/jd_gold/` including table KV, multi-column, unlabeled paragraph, title cases, detailed skills / soft-wrap bullets, unlabeled `Location Mumbai`, video-editor tool skills, wireframing `o` bullets / title-line Mumbai)
...
- **Accuracy note (Current):** synthetic field report `docs/document_intelligence/ACCURACY_REPORT.md` (gold lake) is **not** production accuracy. Prefer JD PDF batch + resume production validation (`ai/eval/resume_production_validation/`) for real-document scores.
...
- JD golden + PDF batch (`tests/backend/test_jd_golden_accuracy.py`, `apps/data/jd_parse_eval/`)
- Resume pattern regressions + optional production validation (`ai/eval/resume_production_validation/`)
- Synthetic gold-lake `ACCURACY_REPORT.md` is **not** a production accuracy claim
...
**Resume parse checklist (always listed, in order):** Cache Check → Store Raw File → Extract Text → Layout Analysis → Section Detection → Deterministic Parse → Semantic Enrichment (LLM) → Knowledge Enrichment → Validation → Save Parsed Result. Optional sub-row: LLM Inference (AI Runtime) when `parse_via_runtime` ran.

**Bulk resume parse (Current):** One dashboard row per bulk job — **Bulk Parse · N resumes** (Bulk filter only). The list row has a chevron to preview filenames; the detail panel lists each resume with an expandable dropdown showing the **same pipeline steps as a single resume parse**. Cache / Store Raw / Layout are typically skipped on the bulk text path.
