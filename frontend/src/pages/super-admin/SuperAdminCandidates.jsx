...
export default function SuperAdminCandidates() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await apiRequest('/api/super-admin/candidates', { method: 'GET', token })
  ...
  const handleDelete = async (cid) => {
    ...
    try {
      const token = tokenService.getToken()
      await apiRequest(`/api/super-admin/candidates/${cid}`, { method: 'DELETE', token })
    ...
    <SuperAdminLayout>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => generateCandidatesPdf(filtered.length ? filtered : candidates)}
              ...
              <tbody className="divide-y divide-zinc-800">
                {filtered.map((c) => (
                  <tr
                    ...
                    role="button"
                    tabIndex={0}
                    onClick={() => navigate(`/super-admin/candidates/${encodeURIComponent(c.cid)}`)}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/super-admin/candidates/${encodeURIComponent(c.cid)}`) } }}
