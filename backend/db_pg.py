...
Use this module when migrating from SQL Server (set USE_POSTGRES=1 or DATABASE_URL=postgresql://...).
"""
import os
...
class ConnectionPool:
    ...
    def get_connection(self, timeout=5):
        if not self._initialized:
            with self.lock:
                if not self._initialized:
                    print(f"[DB PG] Initializing connection pool with {self.pool_size} connections...")
                    for _ in range(self.pool_size):
                        try:
                            conn = self._create_connection()
                            self.pool.put(conn)
                        except Exception as e:
                            print(f"[DB PG WARNING] Could not pre-populate connection: {e}")
...
def run_migrations():
    """Run PostgreSQL schema/migrations from backend/schema_pg/."""
    ...
    with open(schema_file, 'r', encoding='utf-8') as f:
        sql = f.read()
    # Remove full-line comments so text after ";" in comments isn't executed
    lines = []
    for line in sql.splitlines():
        ...
        lines.append(line)
    sql_clean = '\n'.join(lines)
    # Split by semicolon; each statement gets ";" re-appended for execute
            ...
            for stmt in statements:
                try:
                    ...
                    if 'already exists' not in str(e).lower() and 'duplicate' not in str(e).lower():
                        print(f"[DB PG] Migration warning: {e}")
                    conn.rollback()


def init_db():
    """For PostgreSQL, schema is applied via schema_pg/01_schema.sql. Optionally run migrations (idempotent)."""
    run_migrations()
