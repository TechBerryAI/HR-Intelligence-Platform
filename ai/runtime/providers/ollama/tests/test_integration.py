...
runtime:
  ...
  capabilities_dir: {runtime_root.parent / "capabilities"}
  prompts_dir: {runtime_root / "prompts" / "definitions"}
  schemas_dir: {runtime_root / "schemas" / "definitions"}
...
def _patch_ollama_client(chat_content: dict | str = RESUME_RESPONSE):
    ...
    return patch("runtime.providers.ollama.provider.OllamaClient", side_effect=factory)
...
def test_runtime_fails_when_ollama_unavailable(tmp_path: Path) -> None:
    ...
    with patch("runtime.providers.ollama.provider.OllamaClient", side_effect=failing_client):
        runtime = AIRuntime(load_runtime_config(config_file))
