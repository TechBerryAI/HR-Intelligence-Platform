"""Ollama error mapping to runtime provider exceptions."""
...
def map_httpx_error(exc: Exception, *, provider_id: str) -> ProviderError:
    """Translate httpx failures into runtime provider exceptions."""
    ...
    if isinstance(exc, httpx.ConnectError):
        return ProviderNotAvailableError(
            f"Ollama service unreachable: {exc}",
            provider_id=provider_id,
            retryable=True,
        )
...
def map_http_status(status_code: int, message: str, *, provider_id: str) -> ProviderError:
    """Map HTTP status codes to retryable/non-retryable provider errors."""
