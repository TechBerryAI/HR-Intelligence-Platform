...
| 2. Extraction | `extracted/` | JSON per document | Plain text + metadata from `preprocessing/extract/` |
...
```json
{
  "id": "uuid",
  "doc_type": "resume",
  "source_file": "datasets/raw/resumes/example.pdf",
  "source_hash": "sha256:...",
...
JSONL records in `jsonl/` and `benchmark/` follow the schema in [docs/DATA_PIPELINE.md](../docs/DATA_PIPELINE.md).
...
- **Gitignored:** Raw files, intermediate JSON, JSONL (may contain PII)
...
This pipeline serves all AI features that consume document or structured text:
...
| Salary intelligence | `normalized/` + external salary data (future) |

Canonical pipeline reference: [docs/DATA_PIPELINE.md](../docs/DATA_PIPELINE.md).
