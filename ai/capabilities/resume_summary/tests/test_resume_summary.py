"""Capability package tests for resume_summary."""
...
def test_capability_loads() -> None:
    ...
    assert package.id == "resume_summary"
...
def test_capability_has_prompt_template() -> None:
    ...
    assert "resume_summary_v1" in package.prompt_text or "resume_summary" in package.prompt_text


def test_capability_schema_has_id() -> None:
    package = load_capability(CAPABILITY_DIR)
    assert package.schema_doc.get("$id") == "resume_summary_v1"


def test_capability_runtime_config() -> None:
    package = load_capability(CAPABILITY_DIR)
    assert package.runtime_config.preferred_model_alias == "summarizer"
